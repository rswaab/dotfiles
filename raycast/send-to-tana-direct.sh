...
JSON=$(printf '{"workspace":"%s","payload":%s}' "$WORKSPACE" "$CLIP")
...
if [[ "$CODE" == "200" || "$CODE" == "201" ]]; then
  # Expecting echo: { ok, workspace, targetNodeId, nodeCount }
  MSG=$(echo "$BODY" | python3 - <<'PY' 2>/dev/null || true
import json,sys
try:
  o=json.loads(sys.stdin.read())
  if isinstance(o,dict) and o.get("ok"):
    print(f"Sent → {o.get('workspace')}:{o.get('targetNodeId')} ({o.get('nodeCount')} nodes)")
  else:
    print("Sent")
except Exception:
  print("Sent")
