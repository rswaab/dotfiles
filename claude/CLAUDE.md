...
- **Session summary**: Rob writes one **per session** by running a manual wrap command at the end of a working session — it drafts a rich per-session log from in-context memory → `docs/sessions/YYYY-MM-DD-<slug>.md` (per-session, not per-day; a day may have several). In the **ila-app** project the command is **`/ila-wrap`** (grouped with the other `/ila-*` commands); in any other project there's no command yet — write the per-session log directly in that same format. This replaced the `SessionEnd` hook, which **cannot fire in the desktop app** (sessions persist, so there is no "session end" event; the hook still exists and fires on CLI exits, harmlessly). Do **not** assume a summary is written automatically — if Rob asks you to wrap up, or a session is clearly ending, run `/ila-wrap` (in ila-app) or write the log directly. `docs/.carry-forward.md` is no longer auto-updated by the hook; refresh it explicitly if continuity needs it.
